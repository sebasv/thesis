...
class Calibration(object):
    ...
    def calibrate(self, dt=.01, n_simulations=100, T=10, con=None, q=None):
        ...
        def fun(args):
            S, Mu, Ml = self.calculate(*args[:5], np.r_[args[5:8]], args[8])
            fit = max(
        ...
        self.x = optimize.minimize(fun, np.random.exponential(1, 9), method='L-BFGS-B', bounds=[(None, None), (0, None), (0, None), (None, None), (None, None), (None, None), (None, None), (0, None)]) 
        if con is not None:
            con.execute(
                '''insert into heston(qS, kappa_v, xi_v, sigma_v, delta, sS1, sS2, sS3, gamma, p, dt, kappa_r, xi_r, f, fun)
...
def do_calibration(q, t):
    ...
    con.execute('create table if not exists heston(qS real, kappa_v real, xi_v real, sigma_v real, delta real, sS1 real, sS2 real, sS3 real, gamma real, created datetime default current_timestamp, p real, dt real, kappa_r real, xi_r real, f real, fun real)')
    forward_curve = .002
    ...
    df = df[df.index>0]
    df /= 100
    ca = Calibration(0.0212, 0.00445, forward_curve, df)
    while time.time() < t:
